...
真实电路不是均匀的 Rent 分布——90% 的连接在近邻，10% 跨越半个 die。这 10% 才是 3D folding 的目标。为此引入长尾模型：10% 的边不受距离限制、随机连接任意节点。

### 结果（N=1..100，tail=10%）

| Rent p | 对应电路 | 3D 边数 | 2D 线长 | 3D 线长 | 比值 | 收益 | 跨 Die% |
|--------|---------|---------|---------|---------|------|------|---------|
| 0.5 | SRAM | 62602 | 119866 | 162488 | 1.36 | **−35.6%** | 41.0% |
| 0.6 | GPU | 86764 | 193443 | 265480 | 1.37 | −37.2% | 43.8% |
| 0.7 | CPU | 112803 | 295474 | 397530 | 1.35 | −34.5% | 46.3% |
...
- 长尾减轻了亏损（−42%→−35%），但即使 tail=50% 仍为负
- **随机长边不帮忙。** 只有被分割器刻意垂直对齐的长边（TAO 的"前半段层 0、后半段层 1"）才能在 3D 中缩短。随机撒的长边端点分属两层各自角落，3D 距离不降反升
- 实验四的长短线非对称在这里完全成立：90% 的局部边在 3D 中劣化，10% 的随机长边挽救不了大局
- **3D 的线长收益是一个分割优化问题，不是图属性问题。** 只有当分割器能主动选择哪些长边跨层垂直对齐时，收益才可能转正
