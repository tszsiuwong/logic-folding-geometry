...
不以 $p$ 直接生成图，而是用一个更直观的控制变量——**每个单元只连接曼哈顿距离 ≤R 的邻居**。R 越小互联越局部（低 Rent），R 越大越接近完全图（高 Rent）。

### 结果（N=1..100 汇总）

| 半径 | 总边数 | 2D 线长 | 3D 线长 | 比值 | 收益 | 跨 Die% |
|------|--------|---------|---------|------|------|---------|
| R=2 | 23639 | 38563 | 87706 | 2.27 | **−127.4%** | 16.6% |
| R=3 | 42464 | 95038 | 170688 | 1.80 | −79.6% | 23.4% |
| R=5 | 83849 | 281301 | 355025 | 1.26 | −26.2% | 35.2% |
| R=∞ | 166650 | 971077 | 767429 | 0.79 | **+21.0%** | 50.8% |

![Radius](output/plot_radius.png)

### 规律

- **R≤5 时 3D 全线亏损**——因为短距离连接（d₂≤4）在 3D 中会变长（呼应实验四）。当图中不存在长线时，3D 的层间代价成了纯损耗
- **3D 的正向收益完全来自长线（d₂≥6）**——只有 R=∞（全互连）包含足够多的长线，才看到 +21% 的正收益
- 跨 Die 占比随 R 减小而急剧下降：R=∞ 时 50% → R=2 时仅 16.6%
- 这说明 **Rent 约束越低（R 越小），跨 Die 压力和线长收益同步消失**。Agent 拿到低 Rent 设计应当降低 3D 探索优先级
...
**补充（实验六）**：上述阈值在低 Rent（R≤5、局部互联）场景下会失效——3D 收益变为负。Agent 应先评估设计的互联半径，再决定是否进入 3D 分区流程。
...
| **全文缺失** Rent 规则约束 | 简化说明中引入 Rent's Rule（p=1→p∈[0.5,0.7]），实验六用连通半径 R 做代理，验证了"低 Rent 设计 3D 收益消失" |
| **§2.3** 分割器不应盲搜所有设计 | 实验六：R≤5 时 3D 亏损。Agent 应先评估互联半径，低 Rent 设计降低优先级 |
