...
N 个全互连的标准单元（完全图 K_N），按最优空间密铺排列。比较 2D 网格 vs 3D 立方体排布下，总的曼哈顿线长变化。

- **2D**：标准单元挤在尽可能"方"的矩形里 → 平均跳距 O(√N)
- **3D**：标准单元挤在尽可能"立方"的盒子里 → 平均跳距 O(∛N)
...
- **1..100 平均收益：22.4%**
- **峰值收益：34.1%（N=91）**
- 收益随 N 增大呈上升趋势，收敛速度由 $O(N^{-1/6})$ 主导
- N=100 时收益已达 32.6%，继续增大 N 收益将进一步提升
