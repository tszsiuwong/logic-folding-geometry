...
以物理设计工程师 + EDA 视角，**回归第一性原理**（几何、面积、连接密度），回答架构师关于 3D 堆叠的原始问题：
...
本文的作者是物理设计工程师，天天看时序报告。以下距离是真实存在的，不应该被几何模型的简洁性所掩盖：

| 本文做了 | 本文没做 |
|---------|---------|
| 曼哈顿距离的几何推导 | 真实布线拥塞下的绕行惩罚 |
| 单元中心到中心的跳数 | 单元实际 pin 位置、多 pin net 的 HPWL 包围盒 |
| z 方向距离固定为 1 | 不同工艺的层间 RC 延迟（TSV/HB 的寄生远比平面金属大） |
| 完全图（p=1）的上界 | 真实 netlist 的 Rent 约束（p<1） |
| 等面积单元 | 异质单元（不同驱动强度、不同面积） |
| 双 Die 均匀分割 | 时序驱动的非均匀层指派 |
| 面积重叠的几何模型 | 热耦合、IR drop、时钟 skew 的 3D 效应 |

这些缺项不是缺陷，是分阶段逼近的刻意选择：**先用几何确定边界，再用物理填充真实数据**。后续实验将逐项引入——Rent 约束是第一优先级，因为它同时影响所有已有结论的定量精度。
...
> **本文的简化策略**：当前实验取 $p=1$（完全图），是刻意为之的最差情形——线长和 HB 密度都会被高估。这相当于互连复杂度的**上界测试**。后续实验将引入 Rent 约束（$p \in [0.5, 0.7]$）逼近真实电路，预期收益更大、HB 压力更小、阈值更宽。
