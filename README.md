...
以 Rent 指数 p 生成约束图：每个单元连接曼哈顿距离 ≤R 的邻居，R = N^(p/2)。**2D 和 3D 各自使用自己的坐标算边**——3D folding 后位置重映射，原来 2D 中不连通的节点对，在 3D 中因为被拉近了而可能变为连通。

### 结果（N=1..100 汇总）

| Rent p | 对应电路 | 2D 边数 | 2D 线长 | 3D 线长 | 比值 | 收益 | 跨 Die% |
|--------|---------|---------|---------|---------|------|------|---------|
| 0.5 | SRAM | 40483 | 89102 | 126894 | 1.42 | **−42.4%** | 39.1% |
| 0.6 | GPU | 55706 | 148371 | 211759 | 1.43 | −42.7% | 42.1% |
| 0.7 | CPU | 73577 | 231375 | 320843 | 1.39 | −38.7% | 44.6% |
| 1.0 | 全互连 | 166650 | 971077 | 767429 | 0.79 | **+21.0%** | 50.8% |

![Radius](output/plot_radius.png)

### 规律

- 3D 各自算边后亏损减轻（从 −85% 收窄到 −42%），但仍旧全线为负
- **3D folding 确实创造了新的连通性**（跨 Die 占比从 22% 升至 39%）——位置重映射把部分 2D 远距离节点变成了 3D 近邻
- 但层间 +1 代价仍然压在大量局部连接上，短线的劣化大于新增长线的收益
- **p 必须在 1 附近（接近全互连）才能看到正向收益**

> **核心矛盾**：Rent 约束下 3D 线长收益不存在。3D 的真正价值不在线长，而在面积密度和异构集成。如果坚持要拿线长收益，必须主动打破 Rent 约束——即 TAO 分割器的"路径感知切割"本质就是在局部修改有效 Rent 指数。
...
| 边界 | 来源 | 条件 |
|------|------|------|
| 下界 N ≥ 7 | 线长收益 | **仅在 p≈1 时成立**。真实 Rent (p≤0.7) 下 3D 线长收益为负 |
| 上界 N ≤ 4/p² | HB 密度约束 | 超过则跨 Die 连接数超出工艺能力 |

分割器在 [7, 4/p²] 内搜索，但应理解：**在真实 Rent 约束下，3D 的价值不在线长而在面积和集成密度**。p<1 时分割器的目标函数应从"缩短线长"转为"最大化面积利用率 + 最小化 HB 代价"。
...
| **全文缺失** Rent 规则约束 | 实验六：真实 Rent 下 3D 线长收益为负，3D 价值转向面积/密度。Agent 应据此调整目标函数 |
| **§2.3** 分割器不应盲搜所有设计 | 实验六：p≤0.7 时线长亏损。Agent 应先评估 Rent 指数，低 p 设计优先做面积优化而非线长优化 |
