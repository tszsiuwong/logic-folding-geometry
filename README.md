...
几何思想实验系列：**如果 3D 能释放被 2D 物理约束压制的互联复杂度，代价是什么？**

真实 2D 芯片的 Rent 指数集中在 p=0.5~0.7——这是几十年实测数据的统计结论。一个待验证的猜想是：3D 堆叠提供了垂直通路后，部分在 2D 中因线长代价太高而被迫放弃的长线可能重获可行性，有效 p 可以往上走。

本文以 p=1（完全图）作为这个猜想的**上界测试**，从线长、HB 密度、net 组分、长/短线非对称、面积利用率五个维度量化"如果 p 推到顶，代价是多少"，导出分割器的簇大小阈值和 cost function 先验。
...
> **本文的假设**（待验证）：上述 p 值是在 2D 物理设计约束下测得的。3D 堆叠提供了垂直通路后，一些在 2D 中因线长代价太高而被迫压缩的长线变得可行——有效 p 可能比 2D 时代更高。本文以 p=1（完全图）作为这个假设的**上界测试**，问：如果 p 真的能推到 1，线长代价是多少、HB 能不能跟住、长线和短线谁受益更多。分割器的任务，就是在真实设计中根据关键路径的需求**有选择地抬升局部 p**。
