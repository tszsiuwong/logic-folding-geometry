...
> **一个待验证的猜想**：上述 p 值是在 2D 物理设计约束下测得的。3D 堆叠提供了垂直通路后，部分在 2D 中因线长代价太高而被迫放弃的长线可能重获可行性，有效 p 可以往上走。本文以 p=1（完全图）作为这个猜想的**上界测试**——把互联复杂度推到顶，看线长代价多大、HB 能不能跟住、长线和短线谁受益更多。分割器的任务，就是在真实设计中根据关键路径需求**有选择地抬升局部 p**。
...
**分割器 cost function 先验**：d₂≥5 的 net → 跨层激励；d₂≤4 → 保护 Die 内。
...
| §2.3 cost function | d₂≤4 的短线恶化、d₂≥5 的长线受益——分割器应保护短线、优先折叠长线 |
