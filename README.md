...
> **简化说明**：完全图假设是刻意为之的最差情形——线长和 HB 密度都会高估。实际电路遵循 **Rent 规则**：$T = t \cdot B^p$，其中 $p$（Rent 指数）在真实芯片中通常为 0.5~0.7（SRAM≈0.5，CPU≈0.65，GPU≈0.6），远低于完全图对应的 p=1。当前实验先以 p=1 简化，后续将引入 Rent 约束逼近真实互联结构。
