...
将完全图替换为 Rent 约束图：每个单元连接曼哈顿距离 ≤R 的邻居，其中 R = N^(p/2)。p 直接取真实 Rent 指数，不再用代理。

### 结果（N=1..100 汇总）

| Rent p | 对应电路 | 总边数 | 2D 线长 | 3D 线长 | 比值 | 收益 | 跨 Die% |
|--------|---------|--------|---------|---------|------|------|---------|
| 0.5 | SRAM | 40483 | 89102 | 164634 | 1.85 | **−84.8%** | 22.1% |
| 0.6 | GPU | 55706 | 148371 | 234786 | 1.58 | −58.2% | 26.9% |
| 0.7 | CPU | 73577 | 231375 | 314526 | 1.36 | −35.9% | 32.1% |
...
- **真实 Rent 值（p=0.5~0.7）下，3D 全线亏损**——因为局部互联设计中长线极少，层间 z 代价变成纯损耗
- 3D 的正向收益完全来自 p 接近 1 时的长距离互联。p 越小亏损越严重
- 跨 Die 占比随 p 急剧下降：p=1 时 50% → p=0.5 时仅 22%
- **这意味着在真实电路 Rent 约束下，双 Die 堆叠的线长收益不存在——3D 的价值不在缩短线长，而在面积和集成密度**
